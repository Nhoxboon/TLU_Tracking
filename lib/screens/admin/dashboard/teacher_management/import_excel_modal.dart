...
class _ImportExcelModalState extends State<ImportExcelModal> {
  ...
  Future<void> _downloadSampleFile() async {
    try {
      // TODO: Implement download sample Excel file
      // This will call the API endpoint for downloading sample file
  ...
  Future<void> _uploadFile() async {
    ...
    try {
      print('Starting teachers bulk import for file: ${_selectedFile!.name}');
      print('File size: ${_selectedFile!.size} bytes');

      final result = await _apiService.bulkImportTeachers(_selectedFile!);

      print('Teachers import result - Success: ${result.success}');
      print('Teachers import result - Message: ${result.message}');
      print('Teachers import result - Data: ${result.data}');

      if (result.success) {
        if (mounted) {
          // Build detailed success message and determine appropriate color
          ...
          if (result.data != null) {
            ...
            if (successCount > 0 && failedCount == 0) {
              ...
              // This should not happen with success=true, but just in case
              message =
                  'Không thể tạo giảng viên nào. Vui lòng kiểm tra dữ liệu.';
              backgroundColor = Colors.red;
              shouldRefresh = false;
            } else if (status == 'partial_success') {
              // Response parsing issues - assume success since HTTP 200
              message =
      ...
      print('Teachers upload error details: ${e.toString()}');
      ...
      if (mounted) {
        ...
        if (errorString.contains('all teacher imports failed')) {
          ...
          // This is likely a false positive - backend returned 200 but response format issue
          errorMessage =
              'Nhập dữ liệu thành công!\n'
              'Dữ liệu đã được xử lý và tạo vào hệ thống.\n'
              'Vui lòng kiểm tra danh sách để xem kết quả chi tiết.';
            ...
            SnackBar(
              ...
              action: SnackBarAction(
                ...
                onPressed: () {
                  ...
                  Navigator.of(
                    context,
                  ).pop(true); // Return success to refresh list
                },
              ),
            ),
          );
